...
save_folder=sprintf('results_A%dD%d_ep500',dnn.act_a,dnn.act_d);
...
for ff=1:numel(type)
    for ee=1:numel(type)
        for dd=1:numel(type)
            dnn.act_type.forward=type{ff}; 
            dnn.act_type.error=type{ee};    
            dnn.act_type.derivative=type{dd};   
            ...
            for ll=1:dnn.n_layers
                ...
                dnn.nn(ll).weight=sqrt(2/dnn.nn(ll).in_size)*randn(dnn.nn(ll).in_size,dnn.nn(ll).out_size);    
                dnn.nn(ll).bias=zeros(1,dnn.nn(ll).out_size);
                ...
                for batch=1:dnn.num_batches
                    ...
                    for ll=1:dnn.n_layers
                        ...
                        if ll==dnn.n_layers
                            ...
                            [data.nn(ll).z, data.nn(ll).DzDy] = activation(data.nn(ll).y,dnn.act_a,dnn.act_d);  
